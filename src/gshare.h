...
private:
  ...
  std::vector<uint8_t> PHT_; // Pattern History

  uint32_t BHR_; // Branch History Register
  uint32_t BTB_shift_; //Shift for BTB index
  uint32_t BTB_mask_; //mask for BTB index
  uint32_t BHR_mask_; //mask for BHR index
...
private:
  ...
  std::vector<uint8_t> PHT_; // Global PHT
  std::vector<uint8_t> LPHT_; // Local PHT
  std::vector<uint8_t> LHT_; // Local History Table
  std::vector<uint8_t> MetaPredictor_; // Meta Predictor (3-bit fsm)
  std::vector<uint32_t> RAS_; // Return Address Stack
  uint32_t BHR_;
  uint32_t BTB_shift_;
  uint32_t BTB_mask_;
  uint32_t BHR_mask_;
